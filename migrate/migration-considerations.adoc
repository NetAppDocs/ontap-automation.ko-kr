...
---
= ONTAP REST API로 마이그레이션하는 방법에 대해 알아보십시오.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
SnapDiff v1 및 v2는 ONTAPI(ZAPI) 관리 프레임워크를 기반으로 하며, 특정 버전의 ONTAP에서 사용 편의성 제한이 있습니다. SnapDiff v3 API는 ONTAPI를 사용하지 않으며 ONTAP 9.8 이상 버전에서 지원됩니다. SnapDiff ONTAP 릴리스별 API 사용 가능 여부는 다음과 같이 정의됩니다.

* ONTAP 9.7 및 이전 릴리즈: v1 및 v2(ONTAPI만 해당)
* ONTAP 9.8 이상 버전: v1, v2 및 v3. 일부 제한 사항이 적용됩니다. ONTAP 버전에 따른 SnapDiff 지원에 대한 자세한 내용은 https://kb.netapp.com/on-prem/ontap/DP/SnapDiff/SnapDiff-KBs/FAQ_SnapDiff_Support_in_ONTAP["FAQ: SnapDiff ONTAP 지원"^]을 참조하십시오.


백업 애플리케이션이 SnapDiff를 지원하는지 확인하고, 사용 중인 ONTAP 버전에 따른 상호 운용성 제한 사항을 숙지해야 합니다. 자세한 내용은 https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["ONTAP 릴리즈 노트"^]을 참조하십시오.
...
* https://kb.netapp.com/on-prem/ontap/DP/SnapDiff/SnapDiff-KBs/FAQ_SnapDiff_Support_in_ONTAP["FAQ: SnapDiff ONTAP 지원"^]
